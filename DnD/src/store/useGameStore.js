...
const useGameStore = create((set, get) => ({
  ...
  resolveDiceRoll: (sides, resultValue) => {
      ...
      // --- SKILL CHECK ---
          ...
          const checkData = pendingRoll.checkData;

          // Double check 'dc' access.
          const checkDC = checkData.dc || 10;
          const passed = total >= checkDC;

          addToLog({
             text: `Check ${pendingRoll.label}: Rolled ${total} (vs DC ${checkDC}). ${passed ? "Success!" : "Failure!"}`,
             type: 'system'
          });

          set({ pendingRoll: null });

          const nextNode = passed ? checkData.success : checkData.failure;
          if (nextNode) {
              setCurrentNode(nextNode);
          } else {
              console.error("Missing success/failure node target for check", checkData);
          }
      }
  },
  ...
  loadGame: async (uid, characterId) => {
      try {
        ...
        if (data) {
            // Fetch the active adventure content
            let activeAdventure = null;
            if (data.currentAdventureId && import.meta.env.VITE_FIREBASE_API_KEY !== "dummy-key") {
                try {
                    const advSnap = await getDoc(doc(db, 'content_adventures', data.currentAdventureId));
                    if (advSnap.exists()) {
                        activeAdventure = advSnap.data();
                    }
                } catch (e) {
                    console.error("Failed to load active adventure content", e);
                }
            }
